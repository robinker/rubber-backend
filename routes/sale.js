...
router.route('/add').post(async (req, res) => {
    ...
    const source = await User.findOne({firstname: req.body.source.split(' ')[0], lastname: req.body.source.split(' ')[1]})
    const destination = await User.findOne({firstname, lastname})
    // if source and destination are exist
